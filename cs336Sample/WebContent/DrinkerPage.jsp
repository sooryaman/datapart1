...
<html>
	...
	<body>
		...
		<% try {
			...
			String str = "select * from bills b left join transactions t on b.bill_id=t.bill_id where b.drinker = \'"+entity+ "\' group by b.bar order by time asc;";
			ResultSet result = stmt.executeQuery(str);
		%>
		
		<% out.println(entity + "'s Transactions are as follows :") ;%>
	...
	<table>
		...
		<%// out.println("Graphs for "+entity+"\'s Transactions by Day and Month") %>
	...
	try{
		...
		ArrayList<Map<String,Integer>> list3 = new ArrayList();
   		...
   		//Make a query   		
   	   	String	query3 = "select  t.item as beer, count(t.item) as qty from bills b left join transactions t on b.bill_id=t.bill_id where t.type='beer' and b.drinker = \'"+graphType3+"\' group by item order by qty desc limit 5;";
   		...
   		while (result.next()) { 
   			map3=new HashMap<String,Integer>();
   	   		map3.put(result.getString("beer"),result.getInt("qty")); 		
   			list3.add(map3);
   	    } 
   	    result.close();
        ...
        //Create the chart title according to what the user selected
          chartTitle3 = "Transactions by day for "+graphType3;
          legend3 = "Transactions";
...
<div><script src="https://code.highcharts.com/highcharts.js"></script>
	...
	try{
		...
		ArrayList<Map<String,Integer>> list = new ArrayList();
   		...
   		//Make a query   		
   	   	String	query = "select b.day, sum(b.total_price) as Total_Expenditure from bills b where drinker =\'"+graphType+"\' group by b.day;";
   		...
   		while (result.next()) { 
   			map=new HashMap<String,Integer>();
   	   		map.put(result.getString("day"),result.getInt("Total_Expenditure")); 		
   			list.add(map);
   	    } 
   	    result.close();
...
<div><script src="https://code.highcharts.com/highcharts.js"></script>
	...
	try{
		...
		ArrayList<Map<String,Integer>> list2 = new ArrayList();
   		...
   		//Make a query   		
   	   	String	query2 = "select monthname(b.date) as Month, sum(b.total_price) as Total_Expenditure from bills b where drinker =\'"+graphType2+"\' group by Month;";
   		...
   		while (result.next()) { 
   			map2=new HashMap<String,Integer>();
   	   		map2.put(result.getString("Month"),result.getInt("Total_Expenditure")); 		
   			list2.add(map2);
   	    } 
   	    result.close();
        ...
        //Create the chart title according to what the user selected
          chartTitle2 = "Transactions by Month for "+graphType2;
